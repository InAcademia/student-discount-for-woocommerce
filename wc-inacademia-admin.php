...
function inacademia_settings_init() {
	...
	add_settings_section(
		...
		__( 'InAcademia settings.', 'inacademia' ), 'inacademia_section_settings_callback',
		'inacademia'
	);
	...
	if (@$options['redirect_uri_done'] == 'off') unset($options['redirect_uri_done']);
	if (!@$options['redirect_uri_done']) return;
	...
	if (!@$options['client_secret']) return;
...
function sanitize_inacademia_options( $options ) {
	if (is_array($options) and isset($options['coupon_name'])) {
		$options['coupon_name'] = strtolower($options['coupon_name']);
	}
	if (is_array($options) and isset($options['client_id'])) {
		$wrong_client_id = False;
		$client_id = $options['client_id'];
		if (strlen($client_id) != 11) $wrong_client_id = True;
		if (substr($client_id, 0, 3) != 'wc_') $wrong_client_id = True;
		if ($wrong_client_id) {
	...
	if (is_array($options) and isset($options['client_secret'])) {
		$wrong_client_secret = False;
		$client_secret = $options['client_secret'];
		if (strlen($client_secret) != 32) $wrong_client_secret = True;
...
function inacademia_options_page_html() {
  ...
  $tab = isset($_GET['tab']) ? $_GET['tab'] : $default_tab;
    ...
    <nav class="nav-tab-wrapper">
      <a href="?page=inacademia" class="nav-tab <?php if($tab===null):?>nav-tab-active<?php endif; ?>">Important Information</a>
      <a href="?page=inacademia&tab=settings" class="nav-tab <?php if($tab==='settings'):?>nav-tab-active<?php endif; ?>">Settings</a>
      <a href="?page=inacademia&tab=support" class="nav-tab <?php if($tab==='support'):?>nav-tab-active<?php endif; ?>">Support</a>
    </nav>
    ...
    <?php switch($tab):
			case 'settings': ?>
				<form action="options.php" method="post">
					...
					if (@$options['client_secret']) echo esc_html( submit_text() );
